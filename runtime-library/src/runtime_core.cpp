...
static Tensors *build_output(int model_id,
                             const std::vector<Ort::Value> &ort_outputs,
                             int request_id) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (out->tensors[i].data)
      memcpy(out->tensors[i].data,
             ort_outputs[i].GetTensorData<void>(),
             out->tensors[i].data_size);
  }
  return out;
}
    ...
    try {
      ...
      for (int i = 0; i < input->num_tensors; ++i) {
        ...
        ort_inputs.push_back(Ort::Value::CreateTensor(
            mem_info, td.data, data_bytes, shape.data(), shape.size(),
      ...
      auto ort_outputs = m.session->Run(
...
RuntimeStatus runtime_init(Config config) {
  ...
  try {
    ...
    g_env = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_ERROR,
                                       runtime_get_name());
    g_initialized = true;
    ...
    try {
      ...
      try {
        num_threads = std::stoi(
            config_get(mc.config, "num_threads", std::to_string(g_num_threads)));
      ...
      if (mc.model_data && mc.model_size > 0) {
        g_logger->info("[model {}] Loading from memory ({} bytes)", m_idx,
                       mc.model_size);
        ms->session = std::make_unique<Ort::Session>(
            *g_env, mc.model_data, mc.model_size, *ms->session_options);
        ...
        ms->session = std::make_unique<Ort::Session>(*g_env, wpath.c_str(),
                                                     *ms->session_options);
#else
        ms->session = std::make_unique<Ort::Session>(*g_env, mc.file_path,
                                                     *ms->session_options);
#endif
      } else {
        set_error("model_configs[" + std::to_string(m_idx) +
                  "]: file_path and model_data are both null");
        delete ms;
        goto fail;
      }
      ...
      g_logger->info("[model {}] Ready ({} inputs, {} outputs, {} threads)",
                     m_idx, ms->input_names.size(), ms->output_names.size(),
                     num_threads);
    } catch (const std::exception &e) {
      set_error("Failed to load model " + std::to_string(m_idx) + ": " +
                e.what());
      if (ms) delete ms;
      goto fail;
    }
...
RuntimeStatus runtime_retrieve_output(int *model_id,
...
const char *runtime_get_info(void) {
  ...
  g_info_json = "{";
  g_info_json +=
      "\"loaded_models\":" + std::to_string(g_models.size()) + ",";
  g_info_json +=
      "\"requests_in_flight\":" + std::to_string(in_flight) + ",";
  g_info_json +=
      "\"backend_version\":\"" +
      std::string(ort_ver ? ort_ver : "unknown") + "\"";
  g_info_json += "}";
