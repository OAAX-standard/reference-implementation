...
| `CHANGELOG.md` | Every user-facing change; add bullets under the current `# Version X.Y.Z` heading (new heading on version bump) |
| `CLAUDE.md` (root) | If build commands, architecture, threading model, or config keys changed |
| `tests/README.md` | When tests are added, removed, renamed, or change behavior |
| `tests/BENCHMARK_RESULTS.md` | When the threading/perf model changes — mark old results stale or re-run |
| `runtime-library/include/oaax_runtime.h` | Header doc comments are the canonical API reference — config keys listed there must match `config_get()` calls in `runtime_core.cpp` |
...
4. Update `CHANGELOG.md` with a concise bullet under the current version heading.
...
Matches the existing file — one heading per released version, flat bullets:

```markdown
# Version 2.1.0 (2026-07-15)
- Brief description of new capability or change.
...
- Keep READMEs as build/usage guides, not architecture docs (architecture lives in root `CLAUDE.md`)
